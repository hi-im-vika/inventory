...
@app.route('/api/items/<int:item_id>/adjust', methods=['PATCH'])
def adjust_item(item_id):
    item = Item.query.get_or_404(item_id)
    data = request.get_json()
    item.count = max(0, item.count + data['delta'])
    log = CountLog(item_id=item.id, delta=data['delta'], count_after=item.count)
    db.session.add(log)
    db.session.commit()
